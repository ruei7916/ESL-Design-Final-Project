...
public:
	...
	addr_t aes128_0_start_addr = 0x73000000;
	addr_t aes128_size = 0x01000000;
	addr_t aes128_0_end_addr = aes128_0_start_addr + aes128_size - 1;
	addr_t aes128_1_start_addr = 0x74000000;
	addr_t aes128_1_end_addr = aes128_1_start_addr + aes128_size - 1;
	...
	void parse(int argc, char **argv) override {
		...
		assert((mem_end_addr < clint_start_addr || mem_start_addr > aes128_0_end_addr) &&
...
int sc_main(int argc, char **argv) {
	...
	AES128 aes128_0("aes128_0");
	AES128 aes128_1("aes128_1");
	...
	*/
	...
	core0.init(&iss_mem_if0, &iss_mem_if0, &clint, entry_point, rv32_align_address(opt.mem_end_addr-3));
	core1.init(&iss_mem_if1, &iss_mem_if1, &clint, entry_point, rv32_align_address(opt.mem_end_addr-32767));
	...
	bus.ports[12] = new PortMapping(opt.aes128_0_start_addr, opt.aes128_0_end_addr);
	bus.ports[13] = new PortMapping(opt.dma1_start_addr, opt.dma1_end_addr);
	bus.ports[14] = new PortMapping(opt.aes128_1_start_addr, opt.aes128_1_end_addr);


	// connect TLM sockets
	iss_mem_if0.isock.bind(bus.tsocks[0]);
	iss_mem_if1.isock.bind(bus.tsocks[4]);
	dbg_if.isock.bind(bus.tsocks[2]);

	PeripheralWriteConnector dma_connector0("SimpleDMA-Connector0");  // to respect ISS bus locking
	dma_connector0.isock.bind(bus.tsocks[1]);
	...
	bus.isocks[12].bind(aes128_0.tsock);
	bus.isocks[13].bind(dma1.tsock);
	bus.isocks[14].bind(aes128_1.tsock);
